...
class SimplifyUniqueTransformer extends Transform {
  ...
  attributesToProperties(object) {
    if(object.a) {
      let keep = []
      let attrs = Object.keys(object.a)
      while(attrs.length > 0) {
        let attr = attrs.pop()
        if(!object.hasOwnProperty(attr)) {
          object[attr] = object.a[attr]
          delete object.a[attr]
        } else {
          keep.push(attr)
        }
      }
  ...
  childrenToProperties(object) {
    if(object.c) {
      ...
      while(object.c.length > 0) {
        let child = object.c.pop()
        if(!object.hasOwnProperty(child.t)) {
          object[child.t] = child
          delete child.t
        } else {
          keep.push(child)
        }
  ...
  simplifyChildren(object) {
    for(let childIdx in object.c) {
      if(object.c[childIdx] instanceof Object) {
        this.simplify(object.c[childIdx])
      }
    }
  }
