...
class Xml2JsonlProcessor {
  ...
  constructor(args) {
    ...
    this.outputStream = args.outputStream   // the output stream to write to
    this.tags = args.tags                  // optional tags names of the elements to process
    this.filter = args.filter              // optional filter function
  ...
  startElement(name, attrs) {
    ...
    if(this.atroot) {    
      this.atroot = false
      if(this.procroot) {
        this.outputStream.write({t: name, a: attrs})
      }
      return
    }
